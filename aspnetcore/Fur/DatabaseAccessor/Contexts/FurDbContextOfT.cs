...
namespace Fur.DatabaseAccessor.Contexts
{
    ...
    /// Fur 数据库上下文
    /// <para>抽象类，不能实例化</para>
    /// <para>区别于 EF Core 提供的 <see cref="DbContext"/>，<see cref="FurDbContextOfT{TDbContext, TDbContextLocator}"/> 数据库上下文默认继承 <see cref="DbContext"/>，但在此基础上拓展了租户模式，数据库对象类型自动注册 <see cref="DbSet{TEntity}"/>等功能</para>
    ...
    [NonWrapper]
    public abstract class FurDbContextOfT<TDbContext, TDbContextLocator> : DbContext
        ...
        where TDbContextLocator : IDbContextLocator
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            FurDbContextOfTStater.ScanDbObjectsToBuilding(modelBuilder, typeof(TDbContextLocator), this);
