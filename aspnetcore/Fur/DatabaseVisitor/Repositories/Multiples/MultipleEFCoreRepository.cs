...
namespace Fur.DatabaseVisitor.Repositories.Multiples
{
    ...
    public partial class MultipleEFCoreRepository : IMultipleRepository
    {
        ...
        public IMultipleRepositoryOfT<TEntity, TDbContextIdentifier> GetMultipleDbContextRepository<TEntity, TDbContextIdentifier>(bool newScope = false)
            where TEntity : class, IDbEntity, new()
            where TDbContextIdentifier : IDbContextIdentifier
        {
            if (newScope)
            {
                return _lifetimeScope.BeginLifetimeScope().Resolve<IMultipleRepositoryOfT<TEntity, TDbContextIdentifier>>();
            }
            return _lifetimeScope.Resolve<IMultipleRepositoryOfT<TEntity, TDbContextIdentifier>>();
        }
        #endregion
    }
}