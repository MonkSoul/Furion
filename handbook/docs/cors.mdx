...
- `CorsAccessorSettings`
  ...
  - `AllowCredentials`：跨域请求中的凭据，`bool` 类型，默认 `true`
...
很多开发者使用 `SignalR` 进行跨域的时候都会遇到不能跨域的问题，可以参考该 [#I3ALQ7](https://gitee.com/dotnetchina/Furion/issues/I3ALQ7)，具体代码如下：

```cs showLineNumbers {3,6-7,10}
// 注册服务
services.AddSignalR();
services.AddCorsAccessor();
...
app.UseCorsAccessor(builder =>
    builder.SetIsOriginAllowed(_ => true)
           .AllowAnyMethod()
           .AllowAnyHeader()
           .AllowCredentials()  // 注意 必须 AllowCredentials，其他的可以自由配置
);
```
