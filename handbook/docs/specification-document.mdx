...
```cs
services.UseInject(swaggerConfigure: options => {
  // 配置 Swagger Options
...
### 6.5.24 替换 `Swagger` 为 `Knife4jUI`

[knife4j](https://gitee.com/xiaoym/knife4j) 是非常优秀的 `Swagger` UI皮肤库。集成如下：

- 安装 `IGeekFan.AspNetCore.Knife4jUI` 包

- 修改 `Startup.cs` 添加以下配置：
...
namespace FurionApi.Web.Core
{
    public class Startup : AppStartup
    {
        ...
        {
            services.AddControllers()
                    .AddInject(swaggerGenConfigure: configure =>
                    {
                        configure.CustomOperationIds(apiDesc =>
                        {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseInject(swaggerUIConfigure: ui =>
            {
                app.UseKnife4UI(c =>
                {
...
<img src={useBaseUrl("img/sw22.png")} />
<img src={useBaseUrl("img/sw23.png")} />
