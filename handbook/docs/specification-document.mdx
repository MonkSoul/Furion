...
上面使用内存缓存作为例子，当然开发者也可以使用配置文件存储用户名密码方式或 `Session` 或 `Cookies` 验证方式。
...
## 6.5.36 `Swagger` 接口文档支持完整的 `Markdown`
...
```cs showLineNumbers  {15-16,24,35,46,58}
...
namespace YourProject.UnifyResult
{
    ...
    public class YourRESTfulResultProvider  : IUnifyResultProvider
    {
        ...
        {
            return new JsonResult(YourRESTfulResult(StatusCodes.Status400BadRequest, errors: metadata.ValidationResult));
        }
