...
- `LoginInfo`：配置 `Swagger` 是否需要登录才能访问，`SpecificationLoginInfo` 类型，默认 `null`，**仅在 Furion v3.3.1+` 有效**
  - `Enabled`：是否启用登录授权，默认 `false`
  - `CheckUrl`：检查登录状态的 `Url` 地址，**该地址必须是 `GET` 请求**，已授权返回 `200`，否则返回 `401`
