...
关于选择那种类型的筛选器有一个小技巧，**当你不需要全局筛选器的时候使用特性筛选器，否则使用接口类型筛选器**。
...
同时也提供了 `IOrderedFilter` 接口重写执行顺序，其 `Order` 属性值越搞的先执行。
...
通常筛选器支持多个，正常情况下，只要调用 `await next()` 方法都会进去下一个筛选器，但如果通过 `context.Result = new XXXResult()` 之后，就可以使其短路，也就是不会在执行下一个筛选器。
