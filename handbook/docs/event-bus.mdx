...
:::tip 关于 `StackExchange.Redis` 超时问题

如遇到 `StackExchange.Redis` 超时（Timeout）问题，可编辑启动层的 `.csproj` 添加以下配置：
...
有时候我们自定义了事件源存储器如使用 `Redis`、`RabbitMQ` 等第三方存储介质，但由于一些原因导致启动时不可用如 `Redis` 服务未启动，这时候我们可以配置选择回退到默认事件源存储器（内存）中，如：
