...
id: source
title: 1.3 示例结构
sidebar_label: 1.3 示例结构
---

## 1.3.1 示例架构

`Furion` `Samples` 示例创建采用多层分层设计，主要设计是 `自动抽象工厂` 和 `DDD 领域驱动设计` 相结合。
...
## 1.3.6 `Furion.Core` 核心代码层

`Furion.Core` 是核心代码层，存放核心代码，比如仓储、实体、各种通用的东西。
...
## 1.3.10 `Furion.Web.Entry` Web 入口层

`Furion.Web.Entry` 是我们的 Web 入口层，也就是我们的 `Web` 项目层，发布层。
