...
粘土对象是 `Furion` 框架自创的一种概念，是一种可以模拟弱（动态）语言特性的对象，类似 `JavaScript` 一样操作对象。只需通过 `Clay` 类初始化即可。
...
```cs showLineNumbers {12,18,24,33}
dynamic clay = Clay.Object(new
{
    Foo = "json",
    Bar = 100,
...
```cs showLineNumbers {1,10,12,22,24,38}
...
// 设置 Nest 节点不抛异常
clay.Nest.ThrowOnUndefined = true;
var bar = clay.Bar; // => null
```
