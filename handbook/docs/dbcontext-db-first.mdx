...
这时，`Fur Tools` 会自动将 `Fur.Web.Entry` 的 `appsetting.json` 配置数据库连接字符串自动加载进来，`appsetting.json` 配置代码：
...
- `-DbContextLocators`：多数据库上下文定位器，默认 `DbContextLocator`，支持多个，如：`DbContextLocator,MySqlDbContextLocator`
