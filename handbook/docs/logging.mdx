...
| 动态  | -   |   LogDynamic   | 运行时动态日志基本，由 `AppSettings:DynamicLogLevel` 配置决定                                              |
...
如果您的日志是在多线程或后台任务中执行时，就不用使用默认的 `UseSerilogDefault()` 配置了，原因是这个配置是基于请求创建的日志对象。我们需要修改 `Program.cs` 文件，如：
...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        var outputTemplate = "{NewLine}[Time]{Timestamp:yyyy-MM-dd HH:mm:ss,fff}{NewLine}[Level]{Level:u3}{NewLine}[Properties]{Properties:j}{NewLine}[Message]{Message:lj}{NewLine}{Exception}" + new string('-', 50); //输出模板

        Log.Logger = new LoggerConfiguration().ReadFrom
                        .Configuration(configuration).Enrich.FromLogContext()
                        .WriteTo.Console(outputTemplate: outputTemplate).WriteTo.File($"Logs/{date}/.log", LogEventLevel.Information, outputTemplate, rollingInterval: RollingInterval.Day, retainedFileCountLimit: null)
                        .CreateLogger();
