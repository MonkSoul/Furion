...
如果返回大于 `0` 的数值则按照这个数值持续执行，否则取消任务。
...
## 26.11 在 `BackgroundService` 中使用

`BackgroundService` 是 `.NET Core 2` 之后提供的轻量级后台任务，同时可以发布到 `Windows` 服务和 `Linux` 守护进程中。
