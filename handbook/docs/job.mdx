...
这里给出 `SqlSugar` 持久化示例（如果是 `EFCore`，只需要替换成对应的仓储即可）：

```cs showLineNumbers {4,6,25-62,68}
/// <summary>
/// 作业持久化（数据库）
/// </summary>
public class DbJobPersistence : IJobPersistence, IDisposable
{
    ...
    private readonly Repository<SysJobDetails> _jobRepository;
    private readonly Repository<SysJobTriggers> _triggerRepository;
    ...
    public DbJobPersistence(IServiceScopeFactory scopeFactory)
    {
        ...
        _jobRepository = services.GetService<Repository<SysJobDetails>>();
        _triggerRepository = services.GetService<Repository<SysJobTriggers>>();
    }
