...
**当前文档还在编写中，已完成了 97%，将在 2022 年 11 月 23 日完成。**
...
| `Concurrent`         | `bool`      | `true`  | 如果设置为 `false`，那么使用 `串行` 执行                   |
...
```cs showLineNumbers {1,3,8,11,13,17,19}
...
`IJobPersistence` 接口提供了以下三个方法：

- **`Preload`：作业调度器服务启动时调用，通常用来同步持久化数据到内存中。**

在作业调度器服务启动时会自动遍历程序中已注册的作业计划，然后逐条调用该方法，开发者可以在这里通过 `SchedulerBuilder` 修改 `Scheduler` 数据，之后**返回 `SchedulerBuilder` 的持久化行为**。

`SchedulerBuilder` 提供 `Updated()`，`Appended()` 和 `Removed()` 来作为标记，通常标记为 `Updated()`，也就是更新操作。

```cs showLineNumbers {3,8,12-13,16}
public SchedulerBuilder Preload(string jobId, SchedulerBuilder builder)
{
      // 比如这里修改作业信息描述
...
如果存储介质（如数据库）新增了新作业但内存中不存在，开发者可以标记为 `Append()`，这样该作业会添加到内存中。
