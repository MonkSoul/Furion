...
- **问题修复**
  ...
  - &nbsp;<Tag>修复</Tag> 通过 `scheduler.AddTrigger(triggerBuilder)` 无效的问题 <sup>4.8.3.1</sup> <sup>⏱️2022.12.09</sup> [#I65EQ1](https://gitee.com/dotnetchina/Furion/issues/I65EQ1)
...
框架只提供了简单的故障转移的集群功能，如需实现负载均衡，可通过 `TCP/IP` 套接字实现，这里看后续需求，如果需求大，将把完整的负载均衡例子补上。
