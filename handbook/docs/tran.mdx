...
如使用非 `EFCore` ORM 框架，可实现 `IUnitOfWork` 接口之后调用 `services.AddUnitOfWork<TUnitOfWorkd>()` 注册即可，如：

```cs showLineNumbers {8,18,29,40,52,62}
...
```cs showLineNumbers
servers.AddUnitOfWork<SqlSugarUnitOfWork>();
```
