...
```cs {1}
public async Task<List<PersonDto>> GetAll([FromServices] IRepository<Person> repository, string keywork)
{
...
不管采用那种方式，`Fur` 都保证了仓储一次请求唯一性。同时 `App.GetRepository<TEntity>()` 方式支持任何静态类中使用。
