...
通常多文件上传用的最多的是 `List<IFormFile> files` 参数，但 `.NET5+` 更推荐使用 `IFormFileCollection  files`。
...
`Furion` 框架提供了 `400` 多个常见的文件类型 `MIME`，只需要在 `Startup.cs` 或 `Progame.cs` 中注册即可，如：
...
如果配置了还是出现 `404`，那么可能访问的文件类型的 `MIME` 在 `Furion` 框架中未被包含，这时可自行添加，如：
