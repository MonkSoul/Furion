...
所以，实现自动映射赋值和支持特殊配置的需求就有了。目前 `C#` 平台有两个优秀的对象映射工具：`Mapster` 和 `AutoMapper`。**在 `Furion` 框架中，默认集成的是 [Mapster](https://github.com/MapsterMapper/Mapster)，[Mapster](https://github.com/MapsterMapper/Mapster) 是一款极易使用且超高性能的对象映射框架。**
...
所以，社区开发者提供了新的解决方案，可以解决这个问题，代码传送门：[TypeAdapterBuilderExtensions.cs](https://gitee.com/monksoul/Furion/blob/0d3d1dd7768727938a498977057511abc501f75f/framework/Furion.Application/Mapper/TypeAdapterBuilderExtensions.cs)
