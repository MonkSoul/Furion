...
通过 `IEntityTypeBuilder<TEntity>` 接口，我们需要在 `DbContext` 的 `OnModelCreating` 中配置，可在任意地方配置。
...
如果需要跳过自动注册，只需要贴 `[NonAutomatic]` 或 `[NonBeScan]` 特性即可。一旦贴了次特性，那么就需要手动配置 `DbContext` 的 `OnModelCreating`
