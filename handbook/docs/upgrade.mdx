...
- **突破性变化**
  ...
  - &nbsp;<Tag>调整</Tag> **事件总线触发处理程序的逻辑，由过去的 `foreach` 改为 `Parallel.ForEach`，吞吐量提升近 4 倍** <sup>4.6.4</sup>
...
- - &nbsp;<Tag>移除</Tag> **远程请求 `.SetTimeout` 和 `[Timeout]` 配置方法，采用全局统一配置** <sup>4.6.4</sup>
...
- **其他更改**
  ...
  - &nbsp;<Tag>调整</Tag> 优化远程请求性能，添加复用池的机制，避免频繁销毁创建 <sup>4.6.4</sup>
