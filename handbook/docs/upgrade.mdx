...
- **新特性**
  ...
  - [新增] `TP.Wrapper(...)` 拓展方法，主要用来生成规范化的日志模板
...
- **问题修复**
  ...
  - [修复] 框架规范化文档 `Swagger` 不支持 `Controller` 派生类 `api` 路由问题，原生 `ASP.NET` 是支持的
  - [修复] 基于 `Schema` 多租户配置无效问题
  - [修复] 指定实体 `[Table(schema:"dbo")]` 特性后 `Schema` 无效问题
  - [修复] 数据库视图不支持 `Schema` 配置问题
