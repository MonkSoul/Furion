...
- **新特性**
  ...
  - &nbsp;<Tag>新增</Tag> `LogMessage` 结构 `UseUtcTimestamp` 字段，解释日志记录时间格式是 `UTC` 还是 `LOCAL` 时间 <sup>4.6.0</sup> 
  - &nbsp;<Tag>新增</Tag> 事件总线模块重试失败后支持回调 <sup>4.6.1</sup> [#I5UVMV](https://gitee.com/dotnetchina/Furion/issues/I5UVMV)
  - &nbsp;<Tag>新增</Tag> `LoggingMonitor` 支持序列化忽略指定属性名或属性类型 <sup>4.6.1</sup>
