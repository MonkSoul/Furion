...
- **新特性**
  ...
  - &nbsp;<Tag>新增</Tag> **🎉 [全新的定时任务模块](./job.mdx)** <sup>4.8.0</sup> 【[查看源码](https://gitee.com/dotnetchina/Furion/tree/v4/framework/Furion/Schedule)】
  ...
  - &nbsp;<Tag>新增</Tag> 日志 `JSON` 自动美化格式化器 `LoggerFormatter.JsonIndented` <sup>4.8.0</sup>
  - &nbsp;<Tag>新增</Tag> `LoggingMonitor` 的 `JsonIndented` 配置，可配置是否美化 `JSON` <sup>4.8.0</sup>
...
- **文档**
  ...
  - &nbsp;<Tag>新增</Tag> **[新版本定时任务文档](./job.mdx)**
  - &nbsp;<Tag>新增</Tag> `Schedular` 全局静态类文档
  - &nbsp;<Tag>更新</Tag> `Serve.Run()` 入门文档文档、安全授权文档、前端接口代理文档、事件总线文档
