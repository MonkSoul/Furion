...
- **新特性**
  ...
  - &nbsp;<Tag>新增</Tag> `JSON` 序列化 `DateOnly` 和 `TimeOnly` 类型转换器：`.AddDateOnlyConverters()` 和 `.AddTimeOnlyConverters()` <sup>4.7.8</sup> [!657](https://gitee.com/dotnetchina/Furion/pulls/657)
  - &nbsp;<Tag>新增</Tag> `HttpContext.ReadBodyContentAsync()` 拓展方法重复读取 `Body` 内容 <sup>4.7.8</sup> [#I60IYU](https://gitee.com/dotnetchina/Furion/issues/I60IYU)
...
- - &nbsp;<Tag>调整</Tag> **旧版本定时任务为 `弃用` 状态（一周内发布新版），如需取消警告在 `.csproj` 中添加 `<NoWarn>0618</NoWarn>`** <sup>4.7.8</sup> [0ff3ac0](https://gitee.com/dotnetchina/Furion/commit/0ff3ac0fcc54c510ce444b80e9f83eb2c81dc9ba)
...
- **问题修复**
  ...
  - &nbsp;<Tag>修复</Tag> `HttpRequest` 通过 `.ReadBodyContentAsync()` 读取不到 `Body` 问题 <sup>4.7.8</sup> [#I60IYU](https://gitee.com/dotnetchina/Furion/issues/I60IYU)
