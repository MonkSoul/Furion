...
默认情况下，`WinForm/WPF` 不支持依赖注入功能，但 `Furion` 框架提供了完整的支持，**只需要将所有 `new Form()` 或 `new Window()` 替换为 `Native.CreateInstance<Form>()` 或 `Native.CreateInstance<Window>()` 即可**。

### 12.14.1 在 `WinForm` 中使用

- 修改 `Progame.cs` 代码

```cs showLineNumbers {11,14} title="Progame.cs"
using Furion;
...
```cs showLineNumbers {7,12-13,17-18,22}
using Furion;
using Furion.DependencyInjection;
...
public partial class Form1 : Form, IScoped  // 注册为 IScoped 服务
{
...
```cs showLineNumbers {6,10,14-15}
using Furion.DependencyInjection;
using Microsoft.Extensions.Configuration;

namespace WinFormsApp1;

public partial class Form2 : Form, IScoped
{
    private readonly IConfiguration _configuration;
...
public partial class MainWindow : Window, IScoped   // 注册为 IScoped 服务
{
    private readonly IConfiguration _configuration;
    private readonly ITest _test;
...
public partial class Window1 : Window, IScoped
{
    private readonly IConfiguration _configuration;
...
### 12.14.3 `Native.CreateInstance<T>()` 静态方法

`Furion` 框架提供了 `Native.CreateInstance<T>()` 静态方法可在 `WinForm/WPF` 中创建控件、窗口、组件实例，可替代 `new T()` 操作。

如果 `T` 类型标记了生命周期接口（`ITransient`，`IScoped`，`ISingleton`），那么便支持构造函数注入服务操作。
