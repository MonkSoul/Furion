...
在vue3.2 + ts 框架中使用signalr 
    1、安装微软的signalr typescript客户端包，主要用于调用服务端方法，如（Hub中的SendMessage方法）：
       安装命令： npm i @microsoft/signalr @types/node
    2、vue3.2 + ts中连接signalr：
       ...
       <script setup lang="ts">
        ...
        //启动连接并发送消息测试
        connection.start()
        .then(() => connection.send("SendMassgae", "Hello"));
        //注册web端方法以供后端调用
        connection.on("ReciveMessage", reciveMessage);

        const sendMsg = async () => {
            console.log(messages.value)
            await connection.send("SendMassgae", messages.value).catch(function (err) {
                console.log(err)
            });
        }
        参考：https://docs.microsoft.com/zh-cn/aspnet/core/tutorials/signalr-typescript-webpack?view=aspnetcore-6.0&tabs=visual-studio    
### 24.10.3 `.NET` 客户端
