...
模块化是代码的组成的一种方式，模块化系统就像上图乐高玩具一样，一块一块零散积木堆积起一个精彩的世界。每种积木的形状各不相同，功能各不相同，积木与积木直接互相依赖，互相支撑。
...
```json {2,3,4}
{
  "AppSettings": {
    ...
    // "ExternalAssemblies": ["Module1.dll", "Module2.dll", ...]   // 配置加载网站根目录的模块程序集，可不配置
  }
}
```
...
这种方式通常是在网站根目录下放入模块化 `.dll` 文件，无需添加引用可自动加载。

```json {3}
{
  "AppSettings": {
    "ExternalAssemblies": ["Module1.dll", "Module2.dll", ...]
  }
}
```
