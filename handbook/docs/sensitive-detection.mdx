...
title: 30. 脱敏处理
sidebar_label: 30. 脱敏处理（敏感词）
---

:::important 版本说明

以下内容仅限 `Furion 2.4.2 +` 版本使用。
...
在 `Furion` 系统中，`脱敏处理` 指的是对违法词汇或敏感词检测验证。

## 30.2 如何使用

`Furion` 框架内置了一套默认的敏感词脱敏处理机制，并且提供自定义操作。
...
`sensitive-words.txt` 内容格式为每一行标识一个敏感词汇：
...
`Furion` 框架提供了 `[SensitiveDetection]` 验证特性，可以对参数、属性进行脱敏验证，用法和 `[DataValidation]` 一直，如：
...
public class FurionService
{
    ...
    /// <returns></returns>
    public async Task<bool> IsVaild(string text)
    {
        return await _sensitiveDetectionProvider.IsVaildAsync(text);
    }
}
```
...
public class YourSensitiveDetectionProvider : ISensitiveDetectionProvider
{
    ...
    public async Task<IEnumerable<string>> GetWordsAsync()
    {
        // 这里写你敏感词汇数据的来源（如从数据库读取），建议做好缓存操作
    }
    /// <summary>
    /// 判断敏感词是否有效
    /// </summary>
    ...
    {
        // 这里写你如何判断是正常的字符，返回 true 正常，返回 false 表示是个敏感词汇
    }
}
```
...
public void ConfigureServices(IServiceCollection services)
{
    services.AddSensitiveWords<YourSensitiveDetectionProvider>();
}
```
