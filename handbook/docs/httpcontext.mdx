...
以下内容仅限 `Furion 4.7.8 +` 版本使用。

:::

默认情况下，`ASP.NET Core` 不支持重复读取 `Body` 内容，`Furion` 框架提供了拓展方法，需要按照以下步骤操作：

1. **在 `Startup.cs` 的 `Configure` 启用 `Body` 重复度功能**：
...
// HttpContext 拓展
var body = httpContext.ReadBodyContentAsync();

// HttpRequest 拓展
var body = httpContext.Request.ReadBodyContentAsync();
```
