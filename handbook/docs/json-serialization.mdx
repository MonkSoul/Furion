...
如果类型不是 `long` 而是 `long?`，可修改 `long` 为 `long?` 即可。如果两者都要，那么创建一个 `LongNullableJsonConverter` 类并继承 `JsonConverter<long?>`。
...
.AddNewtonsoftJson(options =>
{
     // long类型序列化时转 string ,防止 Javascript 精度溢出
