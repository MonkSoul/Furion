...
### 23.5.12 long类型序列化时转string

有时候我们需要将long类型序列化时转为string类型，防止js出现精度溢出问题，这个时候可以尝试使用以下方法解决：
...
需新建一个类继承`JsonConverter<long>`类并重写Read、Write方法
...
.AddNewtonsoftJson(options =>
{
     //long类型序列化时转string,防止js精度溢出
     options.SerializerSettings.Converters.Add(new LongToStringConverter());
})
```

需新建一个类继承`JsonConverter`类并重写ReadJson、CanConvert、WriteJson方法
