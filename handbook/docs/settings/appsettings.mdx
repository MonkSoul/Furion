...
- `AppSettings`：配置根节点
  ...
  - `EnabledDistributedMemoryCache`：是否启用分布式内存缓存，`bool` 类型，默认 `true`，**如果启用了 Redis 建议关闭**
  ...
  - `EnabledVirtualFileServer`：是否启用虚拟文件服务，`bool` 类型，默认 `true`，**默认脱敏处理依赖此配置**
  - `LogEntityFrameworkCoreSqlExecuteCommand`：是否记录 `EFCore` 执行 `sql` 日志，默认 `true`
