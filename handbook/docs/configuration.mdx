...
推荐使用 《[3.2 选项](options.mdx)》代替本章节功能。（[为什么](#317-配置的优缺点)）？
...
大多情况下，我们的配置只需要在 `appsettings.json` 中配置即可，但一些特殊情况下，我们希望某些组件或功能拥有独立的配置，自定义配置文件支持 `.json` 和 `.xml` 两种方式，比如配置：
...
```cs {2,20} title="Fur.Web.Entry/Program.cs"
...
如果只需要**一次性读取**配置信息，则使用配置，否则应该使用 《[3.2 选项](options.mdx)》代替。
