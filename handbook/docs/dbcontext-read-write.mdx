...
读写分离操作主要使用 `IAdjustableRepository` 仓储，该仓储已经为开发者提供方便的操作调用。当然也可以不使用该仓储。

下面就给大家演示如何读写多库读写操作。

### 8.25.2.1 创建 `主库` 数据库上下文
