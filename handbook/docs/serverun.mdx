...
[https://www.bilibili.com/video/BV17v4y1g7kf](https://www.bilibili.com/video/BV17v4y1g7kf)
...
传入 `委托参数` 相当于自由定义和控制，也就是除了默认集成了 `Furion` 以外，没有注册任何功能。
...
默认情况下 `Serve.Run()` 注册了一些常见的服务和中间件，可查看 [Serve.Run() 更多配置](./serverun.mdx#207-serverun-更多配置) 说明

但不能对这些服务和中间件进行自定义配置，所以，`Furion` 提供了空委托 `Serve.Run(b => {})` + `Startup` 方式，如：

```cs showLineNumbers title="Program.cs"
Serve.Run(b => {}); // 委托写非空默认值即可
```

一个基础的 `Web` 服务 `Startup` 配置：
