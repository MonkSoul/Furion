...
cd HelloFurion
// 添加包
dotnet add package Furion --version 3.6.2
```
...
## 2.0.9 `RunOptions` 和 `LegacyRunOptions`

`Serve.Run` 提供了 `RunOptions` 和 `LegacyRunOptions` 重载参数类型，前者使用的是 `.NET6` `WebApplication` 方式创建主机，后者使用的是 `.NET5` `Host` 方式创建。**优先推荐 `RunOptions` 方式创建。**

这里也列出 `LegacyRunOptions` 的配置例子：

```cs showLineNumbers {4,5,16}
using Microsoft.AspNetCore.Hosting;
using Microsoft.Extensions.DependencyInjection;
