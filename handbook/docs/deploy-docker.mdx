---
id: deploy-docker
title: 34.2 在 Docker 部署
sidebar_label: 34.2 在 Docker 部署
---
...
首先在 `Visual Studio` 或 `dotnet cli` 中发布网站，可以参考 [在 IIS 部署-发布网站](deploy-iis#3411-发布网站)
...
在使用 `.NET8`，默认的端口原来的 `80` 端口变成了 `8080`。[查看相关说明](https://learn.microsoft.com/zh-cn/dotnet/core/compatibility/containers/8.0/aspnet-port)
