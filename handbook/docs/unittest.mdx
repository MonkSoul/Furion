...
**在本章节，`Furion` 框架推荐使用 `xUnit` 库进行单元测试。**
...
上面例子中，我们的测试方法都是没有参数的，有时候我们需要同一个方法输入多个不同的值进行测试，这时候就需要用到 `[Theory]` 和 `[InlineData]` 特性了。

如，下面我们测试两个数的和是 `奇数`，测试代码如下：
...
`Assert` 是单元测试判定成功的依据，通常第一个参数为 `期盼值`，第二个参数为 `实际值`，详细的 `Assert` 静态方法可查阅官方库 [Assert 方法](https://github.com/xunit/assert.xunit)
