...
```cs
var applicationName = App.WebHostEnvironment.ApplicationName;
```

## 1.13 获取网站根目录

```cs
var webRootPath = App.WebHostEnvironment.ContentRootPath;
```
...
```cs
var environmentName = App.WebHostEnvironment.EnvironmentName;
```
...
```cs
// 判断是否开发环境
var isDevelopment = App.WebHostEnvironment.IsDevelopment();

// 判断是否生产环境
var isProduction = App.WebHostEnvironment.IsProduction();

// 判断是否 Stage 环境
var isStaging = App.WebHostEnvironment.IsStaging();

// 判断是否是特定环境，比如自定义测试环境
var isTest = App.WebHostEnvironment.IsEnvironment("TestEnvironment");
```
