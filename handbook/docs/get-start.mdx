...
在 `Fur.EntityFramework.Core` 的 `dbsettings.json` 中配置数据库连接字符串，代码如下：
...
在 `Fur.EntityFramework.Core` 的 `FurEntityFrameworkCoreStartup` 中配置数据库上下文，代码如下：
...
如果你需要安装对应的数据库版本，可以查阅 【[9.18 多数据库操作](dbcontext-multi-database)】 章节。
