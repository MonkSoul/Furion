...
{
    [Get("https://furion.baiqian.ltd/get")]
    Task<Result> GetXXXAsync();

    [Post("https://furion.baiqian.ltd/post")]
    Task<Result> PostXXXAsync();

    [Put("https://furion.baiqian.ltd/put")]
    Task<Result> PutXXXAsync();

    [Delete("https://furion.baiqian.ltd/delete")]
    Task<Result> DeleteXXXAsync();

    [Patch("https://furion.baiqian.ltd/patch")]
    Task<Result> PatchXXXAsync();

    [Head("https://furion.baiqian.ltd/head")]
...
```cs showLineNumbers
var response = await "https://furion.baiqian.ltd/get".GetAsync();

var response = await "https://furion.baiqian.ltd/post".PostAsync();

var response = await "https://furion.baiqian.ltd/put".PutAsync();

var response = await "https://furion.baiqian.ltd/delete".DeleteAsync();

var response = await "https://furion.baiqian.ltd/patch".PatchAsync();

var response = await "https://furion.baiqian.ltd/head".HeadAsync();
```
...
推荐使用 《[19.5 代理方式](http.mdx#195-ihttpdispatchproxy-代理方式)》代替本小节功能。`代理方式` 能够提供更容易且更易维护的方式。
...
// 手动指定发送特定请求
var response = await "https://furion.baiqian.ltd/post".SetHttpMethod(HttpMethod.Post)
                                                  .SendAsync();
```
...
await "".SetRequestUrl("https://furion.baiqian.ltd/");
```

### 19.4.3 设置请求方式

```cs showLineNumbers
await "https://furion.baiqian.ltd/post".SetHttpMethod(HttpMethod.Get);
```
...
```cs showLineNumbers
// 字典方式
await "https://furion.baiqian.ltd/post/{id}?name={name}&id={p.Id}".SetTemplates(new Dictionary<string , object> {
    { "id", 1 },
    { "name", "Furion" },
...
// 对象/匿名对象方式
await "https://furion.baiqian.ltd/post/{id}?name={name}".SetTemplates(new {
    id = 1,
    name = "Furion"
});
```
...
```cs showLineNumbers
// 字典方式
await "https://furion.baiqian.ltd/post".SetHeaders(new Dictionary<string , object> {
    { "Authorization", "Bearer 你的token"},
    { "X-Authorization", "Bearer 你的刷新token"}
});

// 对象/匿名对象方式
await "https://furion.baiqian.ltd/post".SetHeaders(new {
    Authorization = "Bearer 你的token"
});
```
...
```cs showLineNumbers
// 字典方式
await "https://furion.baiqian.ltd/get".SetQueries(new Dictionary<string , object> {
    { "id", 1 },
    { "name", "Furion"}
});

// 对象/匿名对象方式
await "https://furion.baiqian.ltd/get".SetQueries(new {
    id = 1,
    name = "Furion"
});
...
最终输出格式为：`https://furion.baiqian.ltd/get?id=1&name=Furion`。
...
```cs showLineNumbers
await "https://furion.baiqian.ltd".SetClient(() => new HttpClient());
```
...
```cs showLineNumbers
// 传入对象
await "https://furion.baiqian.ltd/api/user/add".SetBody(new User { Id = 1, Name = "Furion" });

// 配置 Content-Type
await "https://furion.baiqian.ltd/api/user/add".SetBody(new { Id = 1, Name = "Furion" }, "application/json");

// 设置 Encoding 编码
await  "https://furion.baiqian.ltd/api/user/add".SetBody(new User { Id = 1, Name = "Furion" }, "application/json", Encoding.UTF8);

// 处理 application/x-www-form-urlencoded 请求
await "https://furion.baiqian.ltd/api/user/add".SetBody(new Dictionary<string , object> {
    { "Id", 1 },
    { "Name", "Furion"}
}, "application/x-www-form-urlencoded");

// 处理 application/xml、text/xml
await "https://furion.baiqian.ltd/api/user/add".SetBody("<SomeDto><SomeTag>somevalue</SomeTag></SomeDto>", "application/xml");
```
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/post".SetContentType("application/json");
```

### 19.4.10 设置内容编码

```cs showLineNumbers
await "https://furion.baiqian.ltd/post".SetContentEncoding(Encoding.UTF8);
```
...
```cs showLineNumbers
// 泛型方式
await "https://furion.baiqian.ltd/api/user/add".SetJsonSerialization<NewtonsoftJsonSerializerProvider>();

// 非泛型方式
await "https://furion.baiqian.ltd/api/user/add".SetJsonSerialization(typeof(NewtonsoftJsonSerializerProvider));

// 添加更多配置
await "https://furion.baiqian.ltd/api/user/add".SetJsonSerialization<NewtonsoftJsonSerializerProvider>(new JsonSerializerSettings {

});

// 比如配置缺省的序列化选项
await "https://furion.baiqian.ltd".SetJsonSerialization(default, new JsonSerializerOptions
    {
        // 中文乱码
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/api/user/add".SetValidationState();

// 设置不验证 null 值
await "https://furion.baiqian.ltd/api/user/add".SetValidationState(includeNull: true);
```
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/".OnRequesting((client, req) => {
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/".OnClientCreating(client => {
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/".OnResponsing((client, res) => {
...
```cs showLineNumbers
await "https://furion.baiqian.ltd/".OnException((client, res, errors) => {
    // res 为 HttpResponseMessage 对象
});
```
...
```cs showLineNumbers
// HttpResponseMessage
var res = await "https://furion.baiqian.ltd/".GetAsync();

// Stream，可用来下载文件
var (stream, encoding) = await "https://furion.baiqian.ltd/".GetAsStreamAsync();

// T
var user = await "https://furion.baiqian.ltd/".GetAsAsync<User>();
...
```cs showLineNumbers
var res = await "https://furion.baiqian.ltd/upload".SetRequestScoped(services);
```
...
```cs showLineNumbers
var res = await "https://furion.baiqian.ltd".SetRetryPolicy(3, 1000).GetAsync();
```
...
```cs showLineNumbers
var res = await "https://furion.baiqian.ltd".WithGZip().GetAsync();
```
...
```cs showLineNumbers
var res = await "https://furion.baiqian.ltd".WithEncodeUrl(false).GetAsync();
```

### 19.4.25 设置 `HTTP` 版本

可解决一些 `HTTP` 和 `HTTPS` 请求问题。

```cs showLineNumbers
var res = await "https://furion.baiqian.ltd".SetHttpVersion("1.0").GetAsync();  // Furion 4.8.5.8+ 支持
```
...
await "https://furion.baiqian.ltd/img/rm1.png".PostToSaveAsync("D:/rm3.png");
...
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://furion.baiqian.ltd/get/{id}?name={name}&number={p.PersonDetail.PhonNumber}")]
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://furion.baiqian.ltd/get/{id}?name={name}"), Headers("key2","value2")]
    Task<HttpResponseMessage> GetXXXAsync(int id, string name);

    [Get("https://furion.baiqian.ltd")]
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://furion.baiqian.ltd/get/{id}?name={name}")]
    Task<HttpResponseMessage> GetXXXAsync(int id, string name);

    [Get("https://furion.baiqian.ltd/get/{p.Id}?name={p.Name}")]
    ...
    [Get("https://furion.baiqian.ltd/get", IgnoreNullValueQueries = true)]
...
[Get("https://furion.baiqian.ltd/get?json={p}", WithEncodeUrl = false)]    // 这里将 p 作为模板传入
Task<HttpResponseMessage> GetXXXAsync(Person p);
```

如果 `Person` 类型不做任何处理，那么最终传递的是 `Person` 的命名空间：`https://furion.baiqian.ltd/get?json=YourProject.Person`，但这并非是我们的预期。

这个时候我们只需要重写 `Person` 的 `ToString` 方法即可，如：
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd/post"), JsonSerialization(typeof(NewtonsoftJsonSerializerProvider))]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd/upload", ContentType = "multipart/form-data")] // bytes 可以通过 File.ReadAllBytes(文件路径) 获取
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd"), RetryPolicy(3, 1000)]    // 支持局部
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd", WithGZip = true)]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd", WithEncodeUrl = false)]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://furion.baiqian.ltd", HttpVersion = "1.1")]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
var response = await "https://furion.baiqian.ltd/".GetAsync();
...
{
    var result = await "https://furion.baiqian.ltd".GetAsStringAsync();
}
```
