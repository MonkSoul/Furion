...
## 19.7 关于PostAsAsync返回值非200时的示例

`Furion` 提供了非常方便的请求并且序列化请求结果`PostAsAsync<T>`
在2.8.8及以下版本，当返回结果的Http状态为非200时，会直接截断。考虑到请求接口的多样性，在2.8.8（不含）以上版本增加无视返回Http状态，直接序列化结果的方式
```cs
//请求并且序列化请求结果
...
如果不加OnException，则会直接截断。
如果需要复杂的Http Post请求，建议直接使用PostAsync,返回值为HttpResponseMessage，可以更灵活的控制结果。
