...
```cs
// 字典方式
await "https://www.furion.pro/post/{id}?name={name}".SetTemplates(new Dictionary<string , object> {
    { "id", 1 },
    { "name", "Furion" }
});
...
有时候我们需要上传文件，需要设置 `Context-Type` 为 `multipart/form-data` 类型，如：
...
public interface IHttp : IHttpDispatchProxy
{
    // 发送 Get 请求
    [Get("https://www.furion.pro/get")]
    Task<HttpRequestMessage> GetXXXAsync();

    // 发送 Post 请求
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync();

    // 发送 Put 请求
    [Put("https://www.furion.pro/put")]
    Task<HttpRequestMessage> PutXXXAsync();

    // 发送 Put 请求
    [Delete("https://www.furion.pro/delete")]
    Task<HttpRequestMessage> DeleteXXXAsync();

    // 发送 Put 请求
    [Patch("https://www.furion.pro/patch")]
    Task<HttpRequestMessage> PatchXXXAsync();

    // 发送 Head 请求
    [Head("https://www.furion.pro/head")]
    Task<HttpRequestMessage> HeadXXXAsync();
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://www.furion.pro/get/{id}?name={name}")]
    Task<HttpRequestMessage> GetXXXAsync(int id, string name);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    ...
    Task<HttpRequestMessage> GetXXX2Async(int id, [Headers]string token = default);

    [Get("https://www.furion.pro")]
    Task<HttpRequestMessage> GetXXX2Async(int id, string name, [Headers("别名")]string token = default);
}
```
...
{
    [Get("https://www.furion.pro/get")]
    Task<HttpRequestMessage> GetXXXAsync([QueryString]int id, [QueryString]string name);

    [Get("https://www.furion.pro/get")]
    Task<HttpRequestMessage> GetXXXAsync([QueryString]int id, [QueryString("别名")]string name);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    ...
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Body("application/x-www-form-urlencoded")]User user);

    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Body("application/x-www-form-urlencoded", "UTF-8")]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    ...
    [Post("https://www.furion.pro/post"), JsonSerialization(typeof(NewtonsoftJsonSerializerProvider))]
    Task<HttpRequestMessage> PostXXXAsync([Body]User user, [JsonSerializerOptions]object jsonSerializerOptions = default);
}
```
...
{
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Range(1,10)]int id, [Required, MaxLength(10)]string name);

    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Required]User user);  // 对象类型支持属性配置特性验证
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    // 通过参数拦截
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Interceptor(InterceptorTypes.Request)] Action<HttpRequestMessage> action = default);
...
public interface IHttp : IHttpDispatchProxy
{
    // 通过参数拦截
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Interceptor(InterceptorTypes.Client)] Action<HttpRequestMessage> action = default);
...
public interface IHttp : IHttpDispatchProxy
{
    // 通过参数拦截
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Interceptor(InterceptorTypes.Response)] Action<HttpResponseMessage> action = default);
...
public interface IHttp : IHttpDispatchProxy
{
    // 通过参数拦截
    [Post("https://www.furion.pro/post")]
    Task<HttpRequestMessage> PostXXXAsync([Interceptor(InterceptorTypes.Exception)] Action<HttpResponseMessage, string> action = default);
