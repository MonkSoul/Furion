...
有些时候，我们通过 `PostMan` 或者第三方工具测试正常，但是使用 `HttpClient` 无法获取结果，比如这个 [#I3W17C](https://gitee.com/dotnetchina/Furion/issues/I3W17C)

这个时候，可能是服务器校验了 `User-Agent` 头，只需要添加 `Headers` 即可，如：

```cs {3}
await "http://xxxxxx/spi?code=S20210422002".OnRequesting(request =>
            {
                request.Headers.Add("User-Agent", "Edge");
            }).GetAsStringAsync();
```
