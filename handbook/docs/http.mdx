...
```cs
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>();
```
...
- `[METHOD]Async/[METHOD]AsTAsync` 方式
...
// 设置请求报文头
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>(headers: new Dictionary<string, string>{
    {"ipaddress", "127.0.0.1"}
});

// 设置请求拦截
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>(interceptor: request=>{
...
// 返回特定类型对象
var data = await "http://47.100.247.61/api/sysdata/categories".PostAsTAsync<object>(new { parm1="", parm2="" });

// 设置请求报文头
var data = await "http://47.100.247.61/api/sysdata/categories".PostAsTAsync<object>(new { parm1="", parm2="" }, headers: new Dictionary<string, string>{
    {"ipaddress", "127.0.0.1"}
});

// 设置请求拦截
var data = await "http://47.100.247.61/api/sysdata/categories".PostAsTAsync<object>(new { parm1="", parm2="" }, interceptor: request=>{
    request.Headers.Add("Authorization","Bearer token字符串");
});
...
// 返回特定类型对象
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>(HttpMethod.Get);

// 设置请求报文头
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>(HttpMethod.Get, headers: new Dictionary<string, string>{
    {"ipaddress", "127.0.0.1"}
});

// 设置请求拦截
var data = await "http://47.100.247.61/api/sysdata/categories".GetAsTAsync<object>(HttpMethod.Post, interceptor: request=>{
    request.Headers.Add("Authorization","Bearer token字符串");
});
```
...
public interface IHttp : IHttpDispatchProxy
{
    ...
    // 请求拦截器
    static HttpRequestMessage RequestInterceptor(HttpRequestMessage httpRequest)
    {
        return httpRequest;
    }

    // 响应拦截器
    static HttpResponseMessage ResponseInterceptor(HttpResponseMessage httpResponse)
    {
        return httpResponse;
    }
}
...
```cs
[Get("api/getdata", ClientName = "github")]
Task<User> GetData();

[Put("api/getdata", ClientName = "facebook")]
Task<User> GetData();
```
...
```cs
// 设置请求拦截
var data = await "http://47.100.247.61/api/sysdata/categories".PostAsTAsync<object>(new { parm1="", parm2="" }, clientName = "github");
```
...
- `[Query]`：自动将参数替换地址中的占位符，占位符格式 `{参数名}`，如：`https://www.furion.pro/user/{id}`，默认基元类型或基元类型数组应用改特性
