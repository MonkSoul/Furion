...
在 `Fur` 框架中，分布式内存缓存服务已经默认注册，无需手动调用 `services.AddDistributedMemoryCache();` 注册。

:::

### 13.4.4 分布式 SQL Server 缓存
