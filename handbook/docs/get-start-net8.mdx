---
id: get-start-net8
title: 2.5 ASP.NET 8 集成
sidebar_label: 2.5 ASP.NET 8 集成 ✨
description: 学习如何在 ASP.NET 8 中集成 Furion
---
...
对 `.NET Core/ASP.NET Core` 有一定基础了解，还未接触的可先看 [【ASP.NET Core 基础】](https://docs.microsoft.com/zh-cn/aspnet/core/fundamentals/?view=aspnetcore-8.0&tabs=windows)
...
使用 `Furion` 之前先确保安装了最新的 `.NET 8 SDK` 并安装 `Visual Studio 2022 Preview` 最新版。

:::

### 2.5.1.1 创建 `ASP.NET Core Web 应用程序`
...
<img src={useBaseUrl("img/getstart9.png")} />
...
<img src={useBaseUrl("img/getstart10.png")} />

## 2.5.3 `Furion` 基本配置

### 2.5.3.1 `Program.cs` 添加 `Inject()`
