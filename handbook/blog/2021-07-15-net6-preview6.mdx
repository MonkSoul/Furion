...
**2021 年 07 月 15 日，微软发布了 `.NET 6 Preview 6` 版本，`Furion` 在当天第一时间适配了该版本，并发布 `Furion v3.0.0-beta.9` 版本。**

[Furion .NET 6 Preview 6 源码地址](https://gitee.com/dotnetchina/Furion/tree/3.0.0-beta.9/)
...
同时升级所有 `Microsoft` 和 `Furion` 包为最新版本，`Furion` 最新版本为：`v3.0.0-beta.9`。
...
`Furion v3.0.0-beta.9` 目前提供了所有类型的脚手架，版本号统一 `3.0.0-beta.9`。

安装脚手架，打开 `CMD/Powershell` 执行以下命令：

```bash
dotnet new --install Furion.Template.Api::3.0.0-beta.9
```
