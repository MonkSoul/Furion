...
>
> 刚好有一次在 QQ 群中无意间刷到了群友发的 **“先知”** 单词：**“`Furion [fi:rɪən]`”**，就那一刻，就认定它了！
>
...
```shell
docker run --name furion -p 5000:80 monksoul/furion:v2.10.7
```
...
```shell
docker build -t furion:v2.10.7 .
```

打包成功后，直接 `docker run`：

```shell
docker run --name furion -p 5000:80 furion:v2.10.7
```
