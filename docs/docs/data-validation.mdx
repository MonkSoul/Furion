...
import sjyz1 from "../static/img/sjyz1.gif";
...
import sjyz2 from "../static/img/sjyz2.gif";
...
import sjyz3 from "../static/img/sjyz3.gif";
...
// 验证时日期或时间格式
"2020-05-20".TryValidate(ValidationLogicOptions.Or, ValidationTypes.Date, ValidationTypes.Time); // => true
"23:45:20".TryValidate(ValidationLogicOptions.Or, ValidationTypes.Date, ValidationTypes.Time); // => true
...
可通过设置 `TryValidate([ValidationLogicOptions], params object[] validationTypes)` 方法的 `ValidationLogicOptions` 参数配置验证逻辑，如：**`同时成立`** 或 **`只要一个成立`** 即可验证通过

:::

### 6.6.3 `[DataValidate]` 特性

`Fur` 还提供了 `[DataValidate]` 特性方便在模型参数中使用 `ValidationTypes` 常见验证或自定义验证。
...
namespace Fur.Application
{
    public class TestDto
    {
        [DataValidate(ValidationTypes.Integer)]
        public int Id { get; set; }

        [DataValidate(ValidationTypes.Numeric, ValidationTypes.Integer)]
        ...
        // 可以和Mvc特性共存
        [Required, DataValidate(ValidationTypes.Age)]
        public int Age { get; set; }

        [DataValidate(ValidationTypes.IDCard, ErrorMessage = "自定义身份证提示消息")]
        public string IDCard { get; set; }
    }
}
```

## 6.7 `[NonValidation]` 跳过验证
...
- 枚举中每一项必须贴有 `[ValidationRegularExpression]` 特性
...
namespace Fur.Application
{
    ...
    public enum MyValidationTypes
    {
        ...
        /// 强密码类型
        /// </summary>
        [ValidationRegularExpression(@"^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$", "必须须包含大小写字母和数字的组合，不能使用特殊字符，长度在8-10之间")]
        StrongPassword,

        /// <summary>
        /// 以 Fur 字符串开头，忽略大小写
        /// </summary>
        [ValidationRegularExpression(@"^(fur).*", "默认提示：必须以Fur字符串开头，忽略大小写", RegexOptions.IgnoreCase)]
        StartWithFurString
    }
}
...
```cs
[DataValidate(MyValidationTypes.StrongPassword)]
public string Password { get; set; }
```
...
### 6.8.2 自定义 `ValidationTypes` 失败消息
...
我们可以通过创建继承 `IValidationTypeErrorMessageProvider` 验证消息提供器类型，或通过 `appsettings.json` 配置。

- **`IValidationTypeErrorMessageProvider` 方式**
...
namespace Fur.Application
{
    public class ValidationTypeErrorMessageProvider : IValidationTypeErrorMessageProvider
    {
        public Dictionary<string, string> Definitions => new Dictionary<string, string>
        {
            // 修改内置验证类型验证失败消息
            [nameof(ValidationTypes.Required)] = "值不能为空或Null",
            [nameof(ValidationTypes.Numeric)] = "必须是数值类型",
            [nameof(ValidationTypes.PositiveNumber)] = "必须是正数",

            // 修改自定义类型验证失败消息
            [nameof(MyValidationTypes.StrongPassword)] = "密码太简单了",
            [nameof(MyValidationTypes.StartWithFurString)] = "必须以 Fur 开头",
        };
    }
}
...
namespace Fur.Web.Entry
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddApp();
            services.AddControllers()
                    .AddDataValidation<ValidationTypeErrorMessageProvider>();
        }
...
import sjyz4 from "../static/img/sjyz4.png";
...
{
  "AppSettings": {
    "ValidationErrorMessageSettings": {
      "Definitions": [
        ["Required", "值不能为空或Null"],
        ["Numeric", "必须是数值类型"],
...
`appsettings.json` 中相同的 `Key` 或覆盖 `IValidationTypeErrorMessageProvider` 提供相同 `Key` 的值。

:::

#### 错误消息查找优先级

`DefaultErrorMessage` -> `IValidationTypeErrorMessageProvider` -> `appsettings.json` (**低 -> 高**)
...
### 6.9.2 `[NonValidation]` 跳过验证

可通过 `[NonValidation]` 贴在 `控制器`，`动作方法`，`类` 中跳过全局验证或不需要验证

### 6.9.3 `[TypeFilter(typeof(DataValidationFilter))]` 局部验证
...
## 6.10 `MiniProfiler` 查看

如下图所示：

import sjyz5 from "../static/img/sjyz5.png";
