...
我们可以通过创建继承 `IValidationTypeMessageProvider` 验证消息提供器类型，或通过 `appsettings.json` 配置。

- **`IValidationTypeMessageProvider` 方式**
...
namespace Fur.Application
{
    public class ValidationTypeMessageProvider : IValidationTypeMessageProvider
    {
...
namespace Fur.Web.Entry
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddApp();
            services.AddControllers()
                    .AddDataValidation<ValidationTypeMessageProvider>();
        }
...
`appsettings.json` 中相同的 `Key` 或覆盖 `IValidationTypeMessageProvider` 提供相同 `Key` 的值。

:::

#### 错误消息查找优先级

`DefaultErrorMessage` -> `IValidationTypeMessageProvider` -> `appsettings.json` (**低 -> 高**)
