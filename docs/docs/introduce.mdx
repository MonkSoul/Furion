...
自 2008年 接触IT这个行业也有十余年了，在 前端（`React/Vue/Angular/Node`）、后端（`.NET/.NET Core/Java/PHP`）、移动端（`Java/Objective-C/Xamarin/小程序`）、桌面端（`WinForm/WPF/Electron`）等主流领域略知一二。

同时在技术培训领域也多有涉足。

**证书方面**

2018 - 2019年 获得 **微软MVP专家** 称号，以及获得 **开源中国GVP最有价值项目** 证书。
