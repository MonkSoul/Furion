...
- `Name`：自定义控制器/动作方法名称，`string`
- `KeepName`：是否保持原有名称不处理，`bool`
- `SplitCamelCase`：切割骆驼命名，`bool`
- `KeepVerb`：是否保留动作方法请求谓词，`bool`
- `Enabled`：是否导出接口，`bool`
- `Module`：模块名，`string`
- `Version`：版本号，`string`
- `Groups`：接口分组，可结合 `Swagger` 一起使用，`string[]`
- `Tags`：接口标签，可结合 `Swagger` 一起使用，`string[]`
- `AuthPolicies`：授权策略，可结合 `授权，JWT` 一起使用，`string[]`
