...
- **不支持引用类型参数在 `GET，HEAD` 请求下生成 `Query` 参数**
...
上述的例子非常简单，创建一个 `FurAppService` 类继承 `IDynamicApiController` 接口，并在这个类中编写一个 `Get` 方法。
...
- 动作方法名以 `Post/Add/Create/Insert/Submit` 开头，则添加 `[HttpPost]` 特性。以 `Get/Find/Fetch/Query/Search` 开头，则添加 `[HttpPost]` 特性。以 `Put/Update` 开头，则添加 `[HttpPut]` 特性。以 `Delete/Remove/Clear` 开头，则添加 `[HttpDelete]` 特性。 **支持自定义配置**
- 如果不在上面约定外，则默认添加 `[HttpPost]` 特性。**支持自定义配置**
