...
namespace Fur.Application
{
    ...
    {
        [ErrorCodeMetadata("{0} 不能小于 {1}")]
        public const int z1000 = 2000;

        [ErrorCodeMetadata("数据不存在")]
        public const int x1000 = 1000;

        [ErrorCodeMetadata("{0} 发现 {1} 个异常", "百小僧", 2)]
        public const string x1001 = "x1001";

        [ErrorCodeMetadata("服务器运行异常")]
...
异常消息类必须是 `公开且是静态类`。`Fur` 框架提供了 `[ErrorCodeType]` 特性和 `IErrorCodeTypeProvider` 提供器接口来提供异常信息扫描，这里用的是 `[ErrorCodeType]` 特性类。

:::

### 5.6.2 关于 `[ErrorCodeMetadata]`

`Fur` 框架提供了`[ErrorCodeMetadata]` 特性用来标识**常量字段**异常元数据，该特性支持传入 `消息内容` 和 `格式化参数`。最终会使用 `String.Format(消息内容，格式化参数)` 进行格式化。
...
namespace Fur.Application
{
    ...
    {
        [ErrorCodeMetadata("用户数据不存在")]
        public const int u1000 = 1000;

        [ErrorCodeMetadata("其他异常")]
...
## 5.8 `IErrorCodeTypeProvider` 提供器
...
使用 `IErrorCodeTypeProvider` 方式需要在 `Startup.cs` 中注册：
...
`[ErrorCodeMetadata]` -> `appsettings.json` -> `[IfException]`。**（低 -> 高）**

- `[IfException]` 会覆盖 `appsettings.json` 定义的状态码消息。
- `appsettings.json` 会覆盖 `[ErrorCodeMetadata]` 定义的消息。
