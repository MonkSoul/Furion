...
link: https://aaa.com/a.mdx
---

:::important 温馨提示

推荐使用 《[2.2 选项](./options.mdx)》代替本章节功能。（[为什么](#216-配置的优缺点)）？
...
假设我们需要在系统运行时获取**系统名称、版本号及版权信息 或 根据不同条件动态配置服务**，而且这些信息可能随时变化。这时就需要将这些信息配置起来。具体步骤如下：
...
- 构造函数中注入 `IConfiguration` 对象读取
- 通过 `App.Configuration[key]` 读取
...
namespace Fur.Web.Entry.Controllers
{
    ...
    public class FurController : ControllerBase
    {
        ...
        public FurController(IConfiguration configuration)
        {
        ...
        public string Get()
        {
            return $@"名称：{_configuration["AppInfo:Name"]}，
                      版本：{_configuration["AppInfo:Version"]}，
                      公司：{_configuration["AppInfo:Company"]}";
        }
    }
}
```
...
- 在可依赖注入类中，选择构造函数注入 `IConfiguration` 读取
- 在静态类/非依赖注入类中，选择 `App.Configuration[key]` 读取
