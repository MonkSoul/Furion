...
```shell
docker run --name fur -p 5000:80 monksoul/fur:v1.0.0-rc.final.60
```
...
```shell
docker build -t fur:v1.0.0-rc.final.60 .
```

打包成功后，直接 `docker run`：

```shell
docker run --name fur -p 5000:80 fur:v1.0.0-rc.final.60
```
